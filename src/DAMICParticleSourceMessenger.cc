...
(DAMICParticleSource *fPtclGun) : fParticleGun(fPtclGun),fShootIon(false) {
  ...
  // particle direction
  directionCmd = new G4UIcmdWith3Vector("/damic/gun/direction",this);
  ...
  depthCmd->SetGuidance("Set laboratory overburden in w.e.");
  depthCmd->SetParameterName("depth",true,true);
  depthCmd->SetDefaultUnit("km");
  //energyCmd->SetUnitCategory("Energy");
  //depthCmd->SetUnitCandidates("km m");

  positionCmd = new G4UIcmdWith3VectorAndUnit("/damic/gun/position",this);
  ...
  typeCmd = new G4UIcmdWithAString("/damic/gun/type",this);
  ...
  // source shape
  shapeCmd = new G4UIcmdWithAString("/damic/gun/shape",this);
  shapeCmd->SetGuidance("Sets source shape type.");
  shapeCmd->SetParameterName("Shape",true,true);
  shapeCmd->SetDefaultValue("NULL");
  shapeCmd->SetCandidates("Sphere Cylinder Cube");
  
  // centre coordinates
  centreCmd = new G4UIcmdWith3VectorAndUnit("/damic/gun/centre",this);
  ...
  halfzCmd = new G4UIcmdWithADoubleAndUnit("/damic/gun/halfz",this);
  ...
  // half height of source
  halfxCmd = new G4UIcmdWithADoubleAndUnit("/damic/gun/halfx",this);
  ...
  // half height of source
  halfyCmd = new G4UIcmdWithADoubleAndUnit("/damic/gun/halfy",this);
  ...
  // radius of source  
  radiusCmd = new G4UIcmdWithADoubleAndUnit("/damic/gun/radius",this);
  ...
  // confine to volume
  confineCmd = new G4UIcmdWithAString("/damic/gun/confine",this);
  ...
  angtypeCmd = new G4UIcmdWithAString("/damic/gun/angtype",this);
  ...
  angtypeCmd->SetCandidates("iso direction Muon");
  
  // energy distribution
  energytypeCmd = new G4UIcmdWithAString("/damic/gun/energytype",this);
...
void DAMICParticleSourceMessenger::SetNewValue
  ...
  fParticleGun->SetHalfX(halfxCmd->GetNewDoubleValue(newValues));
    ...
    G4cout << "pippo: " <<  depthCmd->GetNewDoubleValue(newValues) << G4endl;
  }
  else
